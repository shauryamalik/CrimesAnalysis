...
```
 ...
 - Also, We see a trend increase in the number of shootings in the middle of the year which comes down in every winter.
...
```
 ...
 - Almost every year the number of shootings rise rise sharply after march and then start decreasing in August.
 - Last year, April and may have the highest increase in any month over the pat years.  (possibly due to covid restrictions easing up, and people out of money, had to resort to other means) 
...
```
 ...
 - Also, Brooklyn and Bronx have the most number of shootings which peaked during the month of july in 2020.
...
ggplot(temp_df, aes(x= date)) + 
  ...
  labs(y = "Percentage Change", x="Date", fill="Month", caption = "Source: NYC-open-Data::Shooting Incident Data") +
  theme(plot.title = element_text(face = "bold", size=rel(1.5))) +
  theme(plot.subtitle = element_text(face = "bold", color = "grey35")) +geom_vline(xintercept = covid_date, linetype="dashed", color = "blue") +
  theme(plot.caption = element_text(color = "grey68")) + 
  theme(axis.text.y = element_text(size = rel(1.25)),axis.text.x = element_text(size = rel(1.25)), axis.title.x=element_text(size=rel(1.25)), axis.title.y=element_text(size=rel(1.25)))

```
 
 - This faceted graph gives us a deeper dive in just the highest increase. Staten island and queens show an anomalous increase post covid. much higher than Brooklyn and Bronx. 
...
```
 ...
 - Against the stereotypical belief people going out more on the weekends then weekdays and staying late. This graph shows that the day of the week is does not matter as much for a shooting incident.

### Trend in number of deaths
```{r, fig.width=10,fig.height=11}
temp_df<- df_shoot_grouped_NYC %>% 
  filter(year>=2015, murder==1) %>%
  group_by(date, month) %>%
  summarize(total_shots= sum(occurances))
...
```
 ...
 - This follows from our previous finding of increase in the number of shootings during the same time of the year.
...
```{r}
temp_df<- df_shoot_grouped_BORO %>% 
  filter(year>=2015, VIC_AGE_GROUP!="UNKNOWN") %>%
...
```
 ...
 - The data on number of female victims in shootings is less but among them, brooklyn has the highest.
...
temp_df<- df_shoot_grouped_BORO %>% 
  filter(year>=2015, VIC_RACE!="UNKNOWN", VIC_RACE!="AMERICAN INDIAN/ALASKAN NATIVE") %>%
...
Note: The following regions were missing and their values are set to 0: Pennsylvania, California, Alabama, Iowa, New York, District of Columbia.
...
From the above plots, we see that Texas and Minnesota record the highest number of human trafficking related arrests. A major reason for Texas to be one of the largest rings of labor and sex trafficking is due to its proximity to Mexico. This results in very high illegal immigrants being trafficked for sex, labor, drugs etc. -related purposes.
...
From the above plot, Assault, Felony Assault, Larceny, Drugs among others are the most common offenses seen in New York. These trends are consistent across all 5 boroughs.
...
The following plots are used to understand density of drug-based crimes by location. 
...
It is clear to us that Brooklyn and Bronx have more drug-related arrests, but most importantly Manhattan is where the trend is increasing and has high density.
...
This plot identifies certain locations which tend to have a higher frequency of sexual crimes in NYC. Once again Brooklyn has the larger majority of sex crimes and rape related arrests, but now, we must note that Queens has a very high trend for the same. This can especially be noted for sexual crimes.
