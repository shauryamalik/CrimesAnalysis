...
ggplot(temp_df, aes(x = date, y = VIC_AGE_GROUP)) +
  geom_tile(aes(fill = total_shots), color = "black") +
  theme_heat + ggtitle("Working class of NYC in danger?!",
                       subtitle = "Number of Shootings in NYC for each age group over time") + scale_fill_gradient(low = "#cccccc", high = "#880808") +
  labs(y = "Age Group", x="Date", fill="Number of shootings", caption = "Source: NYC-open-Data::Shooting Incident Data") +
  theme(plot.title = element_text(face = "bold")) +
  theme(plot.subtitle = element_text(face = "bold", color = "grey35")) +
  theme(plot.caption = element_text(color = "grey68"))
```
 ...
 - As expected from the individual trean, even in the overall statistic the people from age group 25-44 are mostly victims in the shootings.
...
```
 ...
 - We observe that different communities are effected more in some boroughs than others.
   ...
   - Pacific Islanders are vicitimezd in queens the highest.
   - White Hispanics and Black hispanica are targeted in Bronx.
   - Black in Brooklyn
...
Please note: 
 ...
 - The following regions were missing and are being set to NA: Pennsylvania, California, Alabama, Iowa, New York, District of Columbia.
...
ggplot(arrests_race, aes(x = fct_reorder(ARREST_BORO, Total), y = Total, fill=PERP_RACE)) +
  ...
  xlab("Race") +
  ylab("Total arrests")+labs(fill='Race')
```
